...
class MigrateV1Command extends Command
{
    ...
    {
        if (!Schema::hasTable('sm_translate_messages')) {
            $this->info("Table [sm_translate_messages] is not found, nothing to migrate.");
            return;
        }

        $langOut = [];
        $messages = Db::table('sm_translate_messages')->pluck('message_data')->all();
