...
class EventPluginRegistry
{
    ...
    /**
     * registerFormFieldAdjustments for SchultenMedia.Pages MenuItem data
     */
    protected function extendStaticPagesBackendFormFields()
    {
        ...
        Event::listen('backend.form.extendFieldsBefore', function($widget) {
            if (!PluginManager::instance()->exists('SchultenMedia.Pages')) {
                return;
            }

            if (!$widget->model instanceof \SchultenMedia\Pages\Classes\Page) {
                return;
            }
    ...
    protected function extendStaticMenusBackendFormFields()
    {
        ...
        Event::listen('backend.form.extendFieldsBefore', function($widget) use ($fieldsToTranslate) {
            // Handle SchultenMedia.Pages MenuItem translations
            if (!PluginManager::instance()->exists('SchultenMedia.Pages')) {
                return;
            }

            if (!$widget->model instanceof \SchultenMedia\Pages\Classes\MenuItem) {
                return;
            }
