...
class TranslatableModel extends TranslatableBehavior
{
    public function __construct($model)
    {
        ...
        $model->morphMany['translations'] = [
            \SchultenMedia\Translate\Models\Attribute::class,
            'name' => 'model'
        ];
        ...
        $model->bindEvent('model.afterDelete', function() use ($model) {
            Db::table('sm_translate_attributes')
                ->where('model_id', $model->getKey())
                ->where('model_type', get_class($model))
                ->delete();

            Db::table('sm_translate_indexes')
                ->where('model_id', $model->getKey())
                ->where('model_type', get_class($model))
                ->delete();
        });
        ...
        foreach ($this->model->$relationGroup as $relationName => $relationObj) {
            ...
            if ($relationClass === \System\Models\File::class) {
                if (is_array($relationObj)) {
                    $this->model->$relationGroup[$relationName][0] = \SchultenMedia\Translate\Models\MLFile::class;
                }
                else {
                    $this->model->$relationGroup[$relationName] = \SchultenMedia\Translate\Models\MLFile::class;
                }
            }
        }
    }
    ...
    /**
     * transWhereInternal
     * @link https://github.com/schultenmedia/translate-plugin/pull/623
     */
    protected function transWhereInternal($query, $index, $value, $options = [])
    {
        ...
        $translateIndexes = Db::table('sm_translate_indexes')
            ->where('sm_translate_indexes.model_type', '=', $this->getClass())
            ->where('sm_translate_indexes.locale', '=', $locale)
            ->where('sm_translate_indexes.item', $index)
            ->where('sm_translate_indexes.value', $operator, $value)
            ->pluck('model_id')
        ;
    ...
    public function scopeTransOrderBy($query, $index, $direction = 'asc', $locale = null)
    {
        ...
        $indexTableAlias = 'sm_translate_indexes_' . $index . '_' . $locale;
    ...
    {
        $joinTableWithAlias = 'sm_translate_indexes as ' . $indexTableAlias;
    ...
    protected function storeTranslatableBasicData($locale = null)
    {
        ...
        $obj = Db::table('sm_translate_attributes')
            ->where('locale', $locale)
        ...
        if ($obj->count() > 0) {
            ...
            Db::table('sm_translate_attributes')->insert([
                'locale' => $locale,
    ...
    protected function storeTranslatableIndexData($locale = null)
    {
        ...
        foreach ($optionedAttributes as $attribute => $options) {
            ...
            $obj = Db::table('sm_translate_indexes')
                ->where('locale', $locale)
                ->where('model_id', $this->model->getKey())
                ->where('model_type', $this->getClass())
            ...
            if ($recordExists) {
                ...
                Db::table('sm_translate_indexes')->insert([
                    'locale' => $locale,
                    'model_id' => $this->model->getKey(),
                    'model_type' => $this->getClass(),
                    'item' => $attribute,
